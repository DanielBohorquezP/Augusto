...
- [x] Títulos acortados en 2 posts que un crawler SEO marcó "title too long" (2026-09-08): "Cómo estructurar...en Colombia" (92 car. con sufijo de marca) → "Cómo estructurar un proyecto de I+D+i trazable" (61 car.); "Crédito fiscal I+D+i y amortización en Colombia: qué cambia" (76 car.) → "Crédito fiscal I+D+i y amortización: qué cambia" (62 car.). El `title` del frontmatter alimenta a la vez `<title>`, H1 y `headline` del schema, así que un solo cambio corrige los tres.
- [x] **Causa raíz de los errores "schema.org validation error" arreglada** (2026-09-08, sitewide, no solo los posts nuevos). El schema global (`Person`, `WebSite`, `ProfessionalService`, `CollegeOrUniversity`) se emitía en un `<script>` aparte desde `app/layout.tsx`, y `articleSchema`/`serviceSchema` referencian esas entidades por `{"@id": "...#person"}` etc. Google resuelve `@id` entre distintos bloques JSON-LD del mismo documento, pero varios validadores de terceros (el crawler SEO que reportó el error) tratan cada `<script>` como un documento aislado y no resuelven la referencia — la marcaban rota en cada página que citaba `author`/`publisher`/`provider`. Fix: nuevo `globalSchemaNodes` en `lib/schema.ts`; se quitó el `SchemaScript` de `layout.tsx` y las 12 páginas (`/`, `/sobre`, `/contacto`, `/politica-privacidad`, `/blog`, `/blog/[slug]`, `/docencia`, `/medios`, `/prime-10` y los 3 `/servicios/*`) ahora incluyen `...globalSchemaNodes` en su propio array de schema, para que `SchemaScript` (que ya fundía todo en un solo `@graph`) emita un único `<script>` por página con todas las referencias resueltas localmente. Verificado con fetch + parse de las 15 rutas: 1 solo script cada una. Si el próximo crawl del SEO tool lo sigue marcando, el problema es otro (revisar campos requeridos por tipo, no las referencias @id).
